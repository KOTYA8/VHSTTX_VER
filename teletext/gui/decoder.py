...
class Decoder(object):
    ...
    def zoom(self, zoom):
        if 0 < zoom < 5:
            small = max(1, int(round(zoom * 10)))
            large = max(1, int(round(zoom * 20)))
            self._fonts[0][0][0].setPixelSize(small)
            self._fonts[0][0][1].setPixelSize(large)
            self._fonts[0][1][0].setPixelSize(small)
            self._fonts[0][1][1].setPixelSize(large)
            self._fonts[1][0][0].setPixelSize(small)
            self._fonts[1][0][1].setPixelSize(large)
            self._fonts[1][1][0].setPixelSize(small)
            self._fonts[1][1][1].setPixelSize(large)
            self.widget.rootContext().setContextProperty('ttfonts', self._fonts)
